...
public class ImpulseBinding extends BoundServiceConnection<ImpulseService> {
     ...
     * device will pause discovery. To stop tracking, call {@link AutoCloseable#close()} on
     * the returned object.
     */
    ...
    private class SendOperation extends Operation<SendListener> implements SendListener {
        ...
        public void onClose() {
            if (mJob != null) {
                ...
                Log.d(LOG_TAG, "SendOperation::onClose");
            }
        }

        @Override // SendListener
        public void onProgress(int total, int sent) {
            Log.d(LOG_TAG, "SendOperation::onProgress");
            mListener.onProgress(total, sent);
        }

        @Override // SendListener
        public void onDone() {
            Log.d(LOG_TAG, "SendOperation::onDone");
            mListener.onDone();
            mJob = null;
            close();
        }
