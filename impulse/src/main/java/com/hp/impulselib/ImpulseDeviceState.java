...
public class ImpulseDeviceState {
    ...
    public static class AccessoryInfo {
        ...
        public AccessoryInfo(byte input[]) {
            ...
            in.get(version);
            firmwareVersion = (version[0] << 16) | (version[1] << 8) | version[2];
            in.get(version);
            hardwareVersion= (version[0] << 16) | (version[1] << 8) | version[2];
        ...
        public String getFWVersionString() {
            return  String.valueOf((0xFF0000 & firmwareVersion) >> 16)
                    + "."
                    + String.valueOf((0x00FF00 & firmwareVersion) >> 8)
                    + "."
                    + String.valueOf(0x0000FF & firmwareVersion);
        }
